...
const main = async () => {
    ...
    app.get(
        ...
        async (_req: Request, res: Response, next: NextFunction) => {
            try {
                res.json({
                    balance: (
                        await Polkadot.balanceOf(polka, polka.faucet.address)
                    ).toString(),
                });
            } catch (e) {
    ...
    app.post(
        ...
        async (
            ...
            try {
                address = new GenericMultiAddress(
                    polka.api.registry,
                    req.body.target
                );
            } catch (_) {
            ...
            const last = (await txrepo.findOne({ address: req.body.target }))
                ?.timestamp;
            const diff = last ? Date.now() - last : Infinity;
            if (diff < min_ms) {
                return next(
                    new HttpError(429, `please retry after ${min_ms - diff}ms`)
                );
            }

            const tx = await Polkadot.transfer(polka, address, amount).catch(
                (e) => (err = e)
            );
            if (err) {
                return next(err);
            ...
            txrepo.persistAndFlush([
                new TransactionInfo(req.body.target, req.body.amount, tx),
            ]);
    ...
    app.listen(config.port, () =>
        console.log(`Express Server is up @ ${config.port}`)
    );
};
